...
reload_kvm_module_for_unsafe_interrupts() {
    ...
    ./evirsh --readonly list | grep running && exit_error "running guest found"
...
create_guest_domain() {
    ...
    prepend_cleanup "./evirsh destroy $1"
    append_cleanup  "./evirsh nodedev-reattach $pci_device_name"
...
destroy_guest_domain() {
    local rc=0
    ./evirsh destroy $1 || ((rc+=1))
    ./evirsh nodedev-reattach $pci_device_name || ((rc+=2))
    return $rc
}

get_guest_domain_pid() {
    # Do not continue if there is no such running domain
    ./evirsh --readonly list | grep $1 > /dev/null || return
    local pid
...
check_selinux_booleans() {
    return 0
    /bin/grep "1 1" /sys/fs/selinux/booleans/virt_use_sysfs
    return $?
}
...
attach_pci_device() {
    ...
    1) # Good case - attached correctly
        ./evirsh attach-device $2 pci_dev.xml || ((rc+=1))
    ;;
...
detach_pci_device() {
    ...
    1) # Good case - detached correctly
        ./evirsh detach-device $2 pci_dev.xml || ((rc+=1))
...
detach_multifunction_devices() {
    ...
    if [ $(./evirsh nodedev-list | grep -c $pcidevice) -gt 1 ]; then
        for dev in $(./evirsh nodedev-list | grep $pcidevice \
            | grep -v $pcifunction); do
            echo "Detaching PCI device $dev from host"
            ./evirsh nodedev-dettach $dev
            [ $? -eq 0 ] && append_cleanup "./evirsh nodedev-reattach $dev"
        done
    fi
}
